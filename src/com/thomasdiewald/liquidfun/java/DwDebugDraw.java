...
/**
 * 
 * Box2D debug draw, utilizing processings basic draw calls.
 * Very useful to test things, but not optimized for performance.
 * 
...
//    this.m_drawFlags |= DwDebugDraw.e_dynamicTreeBit;    // m_contactManager
    ...
    font = papplet.createFont("SourceCodePro-Regular.ttf", 12);
    
    
    // Sprite for rendering Particles
    float particle_rad = world.getParticleRadius() * 2;
    int radius_screen = (int) (Math.ceil(particle_rad * transform.screen_scale) * 2);
    SPRITE = DwUtils.createSprite(papplet, radius_screen, 1, 1, 1);
...
//    System.out.println("DwDebugDraw.drawString: "+s);
  ...
  static public void display(PGraphics pg, World world){
    displayBodies   (pg, world);
    displayParticles(pg, world);
    displayJoints   (pg, world);
  }
